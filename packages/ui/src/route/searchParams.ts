...
export type ParseSearchParamsResult = {
    [x: string]: Primitive | Primitive[] | ParseSearchParamsResult;
};
 ...
 */
export const stringifySearchParams = (params: SearchParamsInput): string => {
    const keys = Object.keys(params);
    if (keys.length === 0) return "";
    // Filter out any keys which are associated with undefined or null values
    const filteredKeys = keys.filter(
        (key: string) => params[key] !== null && params[key] !== undefined,
    );
    const encodedParams = filteredKeys
        .map(
            (key: string) =>
 ...
 * @param params Object with key/value pairs, representing search params
 */
export const addSearchParams = (setLocation: SetLocation, params: SearchParamsInput) => {
 ...
 * @param params Object with key/value pairs, representing search params
 */
export const setSearchParams = (setLocation: SetLocation, params: SearchParamsInput) => {
