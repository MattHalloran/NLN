...
export const resolvers = {
    ...
    Mutation: {
        ...
        updateEmail: async (
            ...
            const curr = await prisma.email.findUnique({
                where: input.id ? { id: input.id } : { emailAddress: input.emailAddress },
            });
            if (!curr || req.businessId !== curr.businessId) {
                throw new CustomError(CODE.Unauthorized);
            }
