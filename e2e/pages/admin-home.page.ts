...
import { APP_LINKS, E2E_TIMEOUTS } from "@local/shared";
...
export class AdminHomePage {
    ...
    async goto() {
        await this.page.goto(APP_LINKS.AdminHomepageHeroBanner);
        await this.page.waitForLoadState("networkidle");
        // Wait for tabs to be fully loaded
        await this.heroTab.waitFor({ state: "visible", timeout: E2E_TIMEOUTS.longMs });
    }
    ...
    async updateBannerAltText(index: number, altText: string) {
        ...
        const altInput = this.page
            .locator(`[data-testid="hero-alt-input-${index}"] input:not([aria-hidden="true"])`)
            .first();
        await altInput.waitFor({ state: "visible", timeout: E2E_TIMEOUTS.shortMs });
    ...
    async updateBannerDescription(index: number, description: string) {
        ...
        const descInput = this.page
            .locator(
                `[data-testid="hero-description-input-${index}"] textarea:not([aria-hidden="true"])`,
            )
            .first();
        await descInput.waitFor({ state: "visible", timeout: E2E_TIMEOUTS.shortMs });
    ...
    async toggleBannerActive(index: number) {
        ...
        const switchControl = this.page.locator(
            `[data-testid="hero-active-switch-${index}"] input[type="checkbox"]`,
        );
        await switchControl.waitFor({ state: "visible", timeout: E2E_TIMEOUTS.shortMs });
    ...
    async deleteBanner(index: number) {
        ...
        await deleteButton.waitFor({ state: "visible", timeout: E2E_TIMEOUTS.shortMs });
    ...
    async dragBanner(fromIndex: number, toIndex: number) {
        ...
        await fromBanner.waitFor({ state: "visible", timeout: E2E_TIMEOUTS.shortMs });
        await toBanner.waitFor({ state: "visible", timeout: E2E_TIMEOUTS.shortMs });
    ...
    async saveChanges() {
        ...
        await this.saveButton.waitFor({ state: "visible", timeout: E2E_TIMEOUTS.mediumMs });
    ...
    async addSeasonalPlant(plantData: {
        ...
        await this.addPlantButton.waitFor({ state: "visible", timeout: E2E_TIMEOUTS.mediumMs });
        await this.addPlantButton.click();

        // Wait for dialog to open
        await this.page.waitForSelector("role=dialog", { timeout: E2E_TIMEOUTS.mediumMs });
        await this.page.waitForTimeout(500);

        // Fill in plant details
        const nameInput = this.page.getByLabel(/^name$/i);
        await nameInput.waitFor({ state: "visible", timeout: E2E_TIMEOUTS.shortMs });
    ...
    async expectSuccessMessage() {
        ...
        await expect(
            this.page
                .locator('[role="alert"], .MuiSnackbar-root, [class*="Snackbar"]')
                .locator("text=/successfully|success/i"),
        ).toBeVisible({ timeout: E2E_TIMEOUTS.extraLongMs });
        // Wait for network activity from the refetch with longer timeout
        await this.page.waitForLoadState("networkidle", { timeout: E2E_TIMEOUTS.longMs });
