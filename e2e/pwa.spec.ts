...
test("registers the service worker and serves the cached app shell offline", async ({ context, page }) => {
  await page.addInitScript(() => {
    ...
    window.addEventListener("nln-service-worker-update-ready", () => {
      (window as typeof window & { __pwaUpdateEvents?: number }).__pwaUpdateEvents =
        ((window as typeof window & { __pwaUpdateEvents?: number }).__pwaUpdateEvents ?? 0) + 1;
    });
  });
...
test("shows a reload action when a service worker update is ready", async ({ page }) => {
  ...
  await page.evaluate(() => {
    window.dispatchEvent(
      new window.CustomEvent("nln-service-worker-update-ready", {
        detail: {
          reload: () => {
            (window as typeof window & { __pwaReloadRequested?: boolean }).__pwaReloadRequested = true;
          },
        },
      }),
    );
  ...
  await expect.poll(() => page.evaluate(() => {
    return (window as typeof window & { __pwaReloadRequested?: boolean }).__pwaReloadRequested;
  })).toBe(true);
});

test("activates a real updated service worker and moves the page to the new version", async ({ page }) => {
  const server = await startServiceWorkerLifecycleServer();
  try {
    await page.goto(server.origin, { waitUntil: "load" });
    await page.waitForFunction(() => navigator.serviceWorker.controller !== null);

    const getControllerVersion = () => page.evaluate(() => {
