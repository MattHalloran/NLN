...
import fs from'node:fs';import path from'node:path';import{receiptEnvelope}from'./scripts/lib/phase10-safe-io.mjs';const[root,h,commit]=process.argv.slice(2),release={version:'10.0.0',commit,releaseId:'fixture'};const write=(name,value)=>fs.writeFileSync(path.join(root,name),JSON.stringify(value),{mode:0o600});
...
const deploy=receiptEnvelope({receiptType:'release-deploy',receiptId:'deploy',status:'success',scope:'fixture',command:'release deploy',release,policy:{id:'release',sha256:h},startedAt:'2026-01-09T11:58:30.000Z',finishedAt:'2026-01-09T12:00:00.000Z'});Object.assign(deploy,{productionMutation:false,userVisibleDowntimeBegan:true,databaseMutationOccurred:false,measuredDowntimeMilliseconds:70000,safestNextAction:'review'});write('deploy.json',deploy);
EOF
}
...
@test "failed and recovered operations remain failed attempts" {
  node --input-type=module - "$WORK/in" "$H" "$C" <<'EOF'
import fs from'node:fs';import path from'node:path';import{receiptEnvelope}from'./scripts/lib/phase10-safe-io.mjs';const[root,h,commit]=process.argv.slice(2),release={version:'10.0.0',commit,releaseId:'fixture'};for(const [index,status]of['failed','blocked','recovered'].entries()){const r=receiptEnvelope({receiptType:'release-deploy',receiptId:`failure-${index}`,status,scope:'fixture',command:'release deploy',release,policy:{id:'release',sha256:h},startedAt:`2026-01-09T12:0${index}:00.000Z`,finishedAt:`2026-01-09T12:0${index}:01.000Z`,failure:{code:'FIXTURE_FAILURE',summary:'fixture failure'}});Object.assign(r,{productionMutation:false,userVisibleDowntimeBegan:false,databaseMutationOccurred:false,measuredDowntimeMilliseconds:0,safestNextAction:'investigate'});fs.writeFileSync(path.join(root,`failure-${index}.json`),JSON.stringify(r),{mode:0o600})}
EOF
  run node scripts/release-observability.mjs summarize --directory "$WORK/in" --output "$WORK/summary.json" --alerts "$WORK/alerts.json" --now "$NOW"
