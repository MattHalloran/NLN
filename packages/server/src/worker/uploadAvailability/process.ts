...
export const findColumnIndex = (header: unknown[], potentialNames: string[]): number => {
    for (const potentialName of potentialNames) {
        const index = header.findIndex(
            (h) => typeof h === "string" && h.trim().toLowerCase() === potentialName.toLowerCase()
        );
        if (index !== -1) {
            return index;
        }
...
export const filterContentRows = (rows: unknown[][]) => {
    return rows
        .filter((row) =>
            row.some(
                (cell) =>
                    typeof cell === "string" && cell.trim() !== "" && !cell.startsWith("Column")
            )
        )
...
export const extractColumnIndices = (header: unknown[]): ColumnIndexMap => {
    return {
        latinName: findColumnIndex(header, [
            "Botanical Name",
            "Botanical",
            "Latin Name",
            "Latin",
        ...
        availability: findColumnIndex(header, [
            "Quantity",
            "Availability",
            "Available",
            "Avail",
...
export const extractLatinNameAndSize = (
    ...
    let latinName: string | null =
        columnIndexMap.latinName !== -1 && typeof row[columnIndexMap.latinName] === "string"
    ...
    const commonName: string | null =
        columnIndexMap.commonName !== -1 && typeof row[columnIndexMap.commonName] === "string"
            ? (row[columnIndexMap.commonName] as string)
            : null;
    if (!latinName && commonName) {
        const existingCommonName = existingCommonNames.find(
            (c) => normalizeString(c.value) === normalizeString(commonName)
        );
        if (existingCommonName) {
            latinName = existingCommonName.plantId;
        }
    }
...
export const upsertPlantTraits = async (
    ...
    for (const key of ["latinName", "commonName"]) {
        ...
        if (typeof traitValue !== "string" || !traitValue.trim()) {
            continue;
        }
        ...
        try {
            ...
            logger.error(`Error upserting plant trait for ${key}:`, error);
        }
    }
};
...
export const upsertSkuData = async (
    ...
    try {
        ...
        logger.error("Error upserting SKU data:", error);
    }
};
...
export const generateSKU = (latinName: string, size: number | null): string => {
    ...
    const randomChars = [...Array(4)]
        ...
        .join(""); // Generates 4 random uppercase letters
...
export const processSkuData = async (
    ...
    // Extract row data
    const note =
        columnIndexMap.note !== -1 && typeof row[columnIndexMap.note] === "string"
            ? (row[columnIndexMap.note] as string)
            : null;
    let sku =
        columnIndexMap.sku !== -1 && typeof row[columnIndexMap.sku] === "string"
    ...
    if (typeof sku !== "string" || (!sku.trim() && size !== null)) {
        const existingSku = existingSkus[plantId]?.find(
            (s) => (s.size ? s.size.toString() : null) === size?.toString()
        );
        if (existingSku) {
            isSkuNew = false;
    ...
    // Handle price
    let price =
        columnIndexMap.price === -1
            ? null
            : parseFloat(`${row[columnIndexMap.price]}`.replace(/[^\d.-]/g, ""));
    ...
    // Handle availability
    let availability =
        columnIndexMap.availability === -1 ? 0 : parseInt(`${row[columnIndexMap.availability]}`);
    if (isNaN(availability)) {
        availability = 0;
    }
    sku_data.availability = availability;
    if (!sku_data.sku) {
        logger.error("⛔️ Did not find SKU data in row:", JSON.stringify(row));
        return;
    }
 ...
 */
export const mergeProcessedSkus = (
    processedSkus: SkuDataMap,
    existingSkus: SkuDataMap
): SkuDataMap => {
...
export const findDuplicateSkus = (existingSkus: SkuDataMap) => {
    ...
    for (const plantId in existingSkus) {
        ...
        const skusGroupedBySize: Record<string, typeof skusForPlant> = {};
        ...
        for (const sizeStr in skusGroupedBySize) {
            ...
            skusForSize.sort((a: (typeof skusForSize)[0], b: (typeof skusForSize)[0]) => {
...
export async function uploadAvailabilityProcess(job: Bull.Job<UploadAvailabilityPayload>) {
    ...
    if (rows.length === 0) {
        logger.warn("⚠️ No rows found in file!");
        return;
    }
    ...
    const existingSkuData = await prisma.sku.findMany({
        select: { sku: true, size: true, plantId: true, availability: true, status: true },
    });
    ...
    for (const sku of existingSkuData) {
        ...
        if (!existingSkus[sku.plantId]) {
            existingSkus[sku.plantId] = [];
        }
    ...
    const duplicateSKUsToDelete = findDuplicateSkus(
        mergeProcessedSkus(processedSkus, existingSkus)
    );
