...
export class AdminContactPage {
    ...
    async goto() {
        await this.page.goto(APP_LINKS.AdminContactInfo);
        await this.page.waitForLoadState("networkidle");
        // Wait for the form to be ready by checking for day checkboxes
        await this.page.waitForSelector("text=/Monday|Tuesday|Wednesday/i", {
            timeout: E2E_TIMEOUTS.extraLongMs,
        });
    }
    ...
    async enableDay(day: string) {
        ...
        await checkbox.waitFor({ state: "visible", timeout: E2E_TIMEOUTS.mediumMs });
    ...
    async setDayHours(day: string, openTime: string, closeTime: string) {
        ...
        await closedCheckbox.waitFor({ state: "visible", timeout: E2E_TIMEOUTS.mediumMs });
        ...
        await openTimeSelect.waitFor({ state: "visible", timeout: E2E_TIMEOUTS.mediumMs });

        // Set opening time
        await openTimeSelect.click();
        await this.page.waitForTimeout(300);
        await this.page
            .locator(`[role="listbox"] [role="option"]:has-text("${openTime}")`)
            .first()
            .click();
        await this.page.waitForTimeout(300);
        ...
        await this.page.waitForTimeout(300);
        await this.page
            .locator(`[role="listbox"] [role="option"]:has-text("${closeTime}")`)
            .first()
            .click();
        await this.page.waitForTimeout(300);
    }
