...
test.describe("Admin Hero Banner Management", () => {
    ...
    test.beforeEach(async ({ authenticatedPage }) => {
        ...
        await authenticatedPage.request.put(`${E2E_SERVER_ORIGIN}${REST_ROUTES.landingPage.root}`, {
            data: {
    ...
    test("should display existing hero banners", async ({
        ...
        test.skip(
            bannerCount === 0,
            "Legacy data-dependent skip: no hero banners were available after setup",
        );
    ...
    test("should edit hero banner alt text and save", async ({ authenticatedPage }) => {
        ...
        await adminPage.switchToHeroTab();
        const reloadedInput = authenticatedPage
            .locator('[data-testid="hero-alt-input-0"] input:not([aria-hidden="true"])')
            .first();
    ...
    test("should edit banner description", async ({ authenticatedPage }) => {
        ...
        await authenticatedPage.waitForLoadState("networkidle");
        await adminPage.switchToHeroTab();
        const reloadedDescInput = authenticatedPage
            .locator('[data-testid="hero-description-input-0"] textarea:not([aria-hidden="true"])')
            .first();
    ...
    test("should toggle banner active status", async ({ authenticatedPage }) => {
        ...
        const firstSwitch = authenticatedPage.locator(
            '[data-testid="hero-active-switch-0"] input[type="checkbox"]',
        );
        ...
        await authenticatedPage.waitForLoadState("networkidle");
        await adminPage.switchToHeroTab();
        const reloadedSwitch = authenticatedPage.locator(
            '[data-testid="hero-active-switch-0"] input[type="checkbox"]',
        );
        if (initialState) {
    ...
    test("should show save/cancel buttons only when changes exist", async ({
        authenticatedPage,
    }) => {
        const bannerCount = await adminPage.getHeroBannerCount();
        test.skip(
...
test.describe("Hero Banner - Advanced Features", () => {
    ...
    test("should reorder banners via drag and drop", async ({ authenticatedPage }) => {
        ...
        test.skip(
            bannerCount < 2,
            "Legacy data-dependent skip: at least 2 hero banners are required to test reordering",
        );
        ...
        const _firstBannerAlt = await authenticatedPage
            ...
            .first()
            .inputValue();
        const secondBannerAlt = await authenticatedPage
            .locator('[data-testid="hero-alt-input-1"] input:not([aria-hidden="true"])')
            .first()
            .inputValue();
        ...
        const newFirstAlt = await authenticatedPage
            .locator('[data-testid="hero-alt-input-0"] input:not([aria-hidden="true"])')
            .first()
            .inputValue();
        ...
        await authenticatedPage.reload();
        await authenticatedPage.waitForLoadState("networkidle");
        const reloadedFirstAlt = await authenticatedPage
            .locator('[data-testid="hero-alt-input-0"] input:not([aria-hidden="true"])')
            .first()
            .inputValue();
