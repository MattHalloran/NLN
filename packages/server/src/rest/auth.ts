...
type AuthRouterLimiters = Pick<
    RateLimiters,
    "loginLimiter" | "passwordResetLimiter" | "signupLimiter"
>;

export type AuthRouterOptions = {
    limiters?: AuthRouterLimiters;
};
...
export function createAuthRouter(options: AuthRouterOptions = {}): Router {
    ...
    const { loginLimiter, passwordResetLimiter, signupLimiter } =
        options.limiters ?? defaultLimiters!;
        ...
        try {
            ...
            if (customer.status in status_to_code) {
                throw new CustomError(
                    status_to_code[customer.status as keyof typeof status_to_code]
                );
            }
        ...
        async (req: Request, res: Response) => {
            try {
                ...
                const verificationExpiry = new Date(
                    Date.now() + AUTH_LIMITS.verificationTokenTtlMs
                );
