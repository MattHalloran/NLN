...
function componentReceipt(file, expected, now) {
    ...
    if (version !== expected.releaseVersion)
        throw new ContractError(`component ${file} has the wrong or missing release`);
    if (commit !== expected.commitSha)
        throw new ContractError(`component ${file} has the wrong or missing commit`);
    if (
        !["fixture", "local"].includes(value.scope) ||
        (expected.scope && value.scope !== expected.scope)
    )
        throw new ContractError(`component ${file} has the wrong or missing scope`);
    return { value, now };
}
...
try {
    if (command === "create") {
        ...
        for (const key of ["identity", "components", "output"])
        ...
        if (
            ...
            throw new ContractError("component manifest is empty or unsupported");
        ...
        for (const [index, item] of manifest.components.entries()) {
            assertExactKeys(
                item,
                { required: ["receiptType", "path"], optional: ["maximumAgeSeconds", "stage"] },
                `component[${index}]`,
            );
            if (seenTypes.has(item.receiptType))
                throw new ContractError(`duplicate component receipt type: ${item.receiptType}`);
            ...
            if (value.receiptType !== item.receiptType)
                throw new ContractError(`component type mismatch for ${absolute}`);
            const finishedAt = value.finishedAt ?? value.generatedAt ?? value.createdAt ?? null;
            if (item.maximumAgeSeconds !== undefined) {
                if (
                    !Number.isSafeInteger(item.maximumAgeSeconds) ||
                    item.maximumAgeSeconds <= 0 ||
                    !finishedAt
                )
                    throw new ContractError(
                        `component ${item.receiptType} has invalid freshness policy`,
                    );
                assertFresh(finishedAt, item.maximumAgeSeconds, now);
            }
            components.push({
                receiptType: item.receiptType,
                stage: item.stage ?? item.receiptType,
                path: absolute,
                sha256: sha256File(absolute),
                releaseVersion: identity.releaseVersion,
                commit: identity.commitSha,
                scope: value.scope ?? "fixture",
                finishedAt,
                maximumAgeSeconds: item.maximumAgeSeconds ?? null,
            });
        }
        components.sort((a, b) => a.receiptType.localeCompare(b.receiptType));
        if (identity.scope === "production")
            throw new ContractError(
                "candidate evidence creation cannot use a production release identity",
            );
        publishJsonNoOverwrite(o.output, {
            schemaVersion: 1,
            receiptType: "release-evidence-index",
            releaseId: identity.releaseId,
            release: { version: identity.releaseVersion, commit: identity.commitSha },
            scope: identity.scope,
        ...
        for (const item of index.components) {
            if (seen.has(item.receiptType))
                throw new ContractError(`duplicate indexed type: ${item.receiptType}`);
            seen.add(item.receiptType);
            regularFile(item.path, `indexed ${item.receiptType}`);
            if (sha256File(item.path) !== item.sha256)
                throw new ContractError(`component hash mismatch: ${item.receiptType}`);
            if (item.scope !== index.scope)
                throw new ContractError(`indexed scope mismatch: ${item.receiptType}`);
            const { value } = componentReceipt(
                item.path,
                {
            ...
            if (value.receiptType !== item.receiptType)
                throw new ContractError(`component type mismatch: ${item.receiptType}`);
            if (item.maximumAgeSeconds !== null)
                assertFresh(item.finishedAt, item.maximumAgeSeconds, now);
        }
        console.log(
            `Release evidence verified: ${index.releaseId} (${index.components.length} components)`,
        );
    } else if (command === "discover") {
        const o = parseOptions(argv);
        if (!o.directory || !o["release-id"])
            throw new ContractError("--directory and --release-id are required", 2);
        const matches = [];
        for (const name of fs.readdirSync(o.directory)) {
            ...
            try {
                ...
                if (
                    value.releaseId === o["release-id"] &&
                    (!o.stage ||
                        value.stage === o.stage ||
                        value.components?.some((x) => x.stage === o.stage))
                )
