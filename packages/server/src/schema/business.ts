...
export const resolvers = {
    Query: {
        ...
        ): Promise<RecursivePartial<Business>[]> => {
            // Must be admin
            if (!req.isAdmin) {
    ...
    Mutation: {
        ...
        updateBusiness: async (
            ...
            if (!req.isAdmin || req.businessId !== input.id) {
                throw new CustomError(CODE.Unauthorized);
            }
        ...
        deleteBusinesses: async (
            _parent: undefined,
            { input }: IWrap<DeleteManyInput>,
            { prisma, req }: Context
        ): Promise<Count> => {
            // Must be admin, or deleting your own
            if (!req.isAdmin || input.ids.length > 1 || req.businessId !== input.ids[0]) {
                throw new CustomError(CODE.Unauthorized);
            }
