        ...
        plants: async (
            ...
            if (showActive) {
                plants = plants.map((p: any) => {
                    // TODO: Type as Plant with SKUs
                    return {
                        ...p,
                        skus: p.skus?.filter((s: any) => s.status === SKU_STATUS.Active) || [], // TODO: Type as Sku
                    };
                });
                plants = plants.filter((p: any) => p.skus.length > 0); // TODO: Type as Plant
            }
            ...
            if (input.onlyInStock === true) {
                plants = plants.map((p: any) => {
                    ...
                    return {
                        ...p,
                        skus: p.skus?.filter((s: any) => s.availability > 0) || [], // TODO: Type as Sku
                    };
                });
                plants = plants.filter((p: any) => p.skus.length > 0); // TODO: Type as Plant
            }
    ...
    Mutation: {
        ...
        addPlant: async (
            ...
            const plant = await prisma.plant.create({
                data: { id: input.id, latinName: input.latinName },
                ...new PrismaSelect(info).value,
            });
            ...
            for (const trait of input.traits || []) {
                ...
                await prisma.plant_trait.create({
                    data: { plantId: plant.id, name: trait.name, value: trait.value },
                });
            }
            // Create images
        ...
        updatePlant: async (
            ...
            if (Array.isArray(input.images)) {
                ...
                await prisma.plant_images.deleteMany({
                    where: {
                        AND: [{ plantId: input.id }, { NOT: { hash: { in: imgHashes } } }],
                    },
                });
                ...
                const operations: any[] = []; // TODO: Type as proper Prisma operations
                for (let i = 0; i < input.images.length; i++) {
                    const existingImage = await prisma.plant_images.findFirst({
                        where: { hash: input.images[i].hash },
                    });
                    if (existingImage) {
                        operations.push(
            ...
            // Update traits
            const currentTraits = await prisma.plant_trait.findMany({
            ...
            for (const trait of inputTraits) {
                ...
                if (existingTrait) {
                    await prisma.plant_trait.update({
                ...
                await prisma.plant_trait.create({
                    data: { plantId: input.id, name: trait.name, value: trait.value },
                });
            }
            // Update SKUs
            if (input.skus) {
                const currSkus = await prisma.sku.findMany({ where: { plantId: input.id } });
                const deletedSkus = currSkus
                    .map((s) => s.sku)
                    .filter((s) => !input.skus!.some((sku: any) => sku.sku === s));
