...
@test "qualification binds two gates contracts fixture chain tests and usability" { qualify;[ "$status" -eq 0 ];[ "$(stat -c %a "$WORK/qualification.json")" = 600 ];node -e 'const r=require(process.argv[1]);if(r.status!=="qualified"||r.productionIntegrationEnabled||r.trustedGateReceipts.length!==2||r.productionObservations.status!=="skipped")process.exit(1)' "$WORK/qualification.json"; }
